...
async function loadStream(streamUrl) {
  ...
  /* ── Extract the hdntl token from the source URL ──
     The source URL has ?hdntl=TOKEN baked in (from auto_streams).
     When Shaka fetches the media playlist and resolves relative segment
     URLs (e.g. seg001.ts), those segment URLs WON'T have the hdntl
     query param. We need to re-append it to every CDN request. */
  let hdntlToken = "";
  try {
    const srcParsed = new URL(streamUrl);
    hdntlToken = srcParsed.searchParams.get("hdntl") || "";
  } catch(e) {}
  ...
  if (shaka.Player.isBrowserSupported()) {
    ...
    /* ── Append hdntl token to every CDN segment/playlist request ──
       Referer and User-Agent are forbidden browser headers — JS cannot
       set them. The only reliable way to authenticate with Akamai from
       a browser is to send the token as a query parameter on each URL.
       We inject it here if the URL doesn't already have it. */
    if (hdntlToken) {
      shakaPlayer.getNetworkingEngine().registerRequestFilter((type, request) => {
        try {
          const u = new URL(request.uris[0]);
          if (u.hostname.includes("fancode.com") && !u.searchParams.has("hdntl")) {
            u.searchParams.set("hdntl", hdntlToken);
    ...
    try {
      /* Pass MIME type hint so Shaka doesn't have to guess */
      await shakaPlayer.load(streamUrl, null, 'application/x-mpegurl');
      showLoading(false);
      video.play().catch(() => {
        video.muted = true;
        video.play();
