...
async function loadStream(streamUrl) {
  ...
  if (shaka.Player.isBrowserSupported()) {
    ...
    /* ── Set required headers for FanCode's Akamai CDN ──
       The JSON explicitly specifies these headers are required.
       Referer MUST be https://fancode.com/ or the CDN returns 403. */
    shakaPlayer.getNetworkingEngine().registerRequestFilter((type, request) => {
      request.headers['Referer'] = 'https://fancode.com/';
      request.headers['User-Agent'] = 'ReactNativeVideo/9.7.0 (Linux;Android 10) AndroidXMedia3/1.6.1';
    });
