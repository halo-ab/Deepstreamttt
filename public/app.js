...
/* Route ALL stream requests through the /api/hls proxy.
   Direct loading causes CORS errors — FanCode CDN restricts cross-origin
   access in browsers. The Worker proxy adds the correct CORS headers. */
...
function playChannel(ch) {
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, () => {
      showLoading(false);
      /* Muted autoplay always works; we unmute immediately after play starts.
         This bypasses browser autoplay-policy blocks. */
      video.muted = true;
      video.play()
