...
/* Load stream directly from browser — same approach as the working IOS/ player.
   Proxy is blocked (all Cloudflare IPs return 403 from FanCode's Akamai CDN).
   Browser residential IPs can access the CDN with CORS open. */
  ...
  for (const src of sources) {
    try {
      ...
      /* /api/events returns { matches, lastUpdate }
         kajju027 JSON returns { matches: [...], live_matches: N, ... } */
      const raw = data.matches || [];
      const live = raw.filter((m) => m.status === "LIVE" || m.type === "live");
...
function parseMatches(matches) {
  return matches.map((m) => {
    /* Use the master manifest (primary stream) so hls.js can select quality.
       Fall back through available URL fields. */
...
function playChannel(ch) {
  ...
  if (Hls.isSupported()) {
    ...
    hls = new Hls({
      ...
      /* Match the exact config of the known-working IOS/ player */
      lowLatencyMode:       true,
      backBufferLength:     90,
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, (_ev, data) => {
      showLoading(false);
      /* Default to 720p like the working player — not auto (1080p) */
      const heights  = data.levels.map((l) => l.height);
      const prefer   = [720, 480, 360, 240];
      const target   = prefer.find((h) => heights.includes(h)) ?? -1;
      if (target !== -1) hls.currentLevel = heights.indexOf(target);
