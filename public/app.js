      ...
      if (live.length > 0) {
        const channels = live
          ...
          .map((m) => {
            ...
            if (language && group.toUpperCase().startsWith(language.toUpperCase() + " ")) {
              group = group.substring(language.length + 1).trim();
            }
            return {
              url:       m.cnptv_cdn,
...
function parseMatches(matches) {
  return matches.map((m) => {
    ...
    /* FanCode API often prepends language to category (e.g. "ENGLISH Cricket") */
    if (language && group.toUpperCase().startsWith(language.toUpperCase() + " ")) {
      group = group.substring(language.length + 1).trim();
    }
