    ...
    if (res.ok) {
      ...
      if (live.length > 0) {
        const channels = live
          ...
          .map((m) => {
            let group = m.category || "Sports";
            let language = m.language || "";
            /* Strip known language prefixes from category regardless of m.language */
            group = group.replace(/^(ENGLISH|HINDI|TAMIL|TELUGU|KANNADA|MALAYALAM|BENGALI|PUNJABI|GUJARATI|MARATHI)\s+/i, "").trim();
  ...
  return matches.map((m) => {
    ...
    if (m.auto_streams) {
      ...
      if (autoData && autoData.auto) {
        ...
        const baseUrlMatch = fallbackUrl.match(/^(https?:\/\/[^\/]+\/.*\/)/);
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.startsWith("#EXT-X-STREAM-INF:")) {
            ...
            if (resMatch && nextLine && !nextLine.startsWith("#")) {
              ...
              /* Fix double slashes (e.g. //720p.m3u8) which cause CDN 403.
                 Avoid lookbehind (?<!:) for better browser compatibility. */
              nextLine = nextLine.replace(/([^:])\/\/+/g, "$1/");
              
              qualities[height] = nextLine;
            }
          }
        }
      }
    ...
    let group = m.category || m.event_category || "Sports";
    let language = m.language || "";
...
/** Show the quality picker modal, then play selected quality */
function showQualityModal(ch) {
  const overlay  = document.getElementById("quality-modal");
  const optsCont = document.getElementById("quality-modal-options");
  const subText  = document.getElementById("quality-modal-sub");
  const cancelBtn = document.getElementById("quality-modal-cancel");
  if (!overlay || !optsCont) return;

  const q = ch.qualities || {};
  const keys = Object.keys(q);

  /* If only one quality (or none), skip the modal and play directly */
  if (keys.length <= 1) {
    _startPlay(ch, ch.url);
    return;
  }

  /* Populate subtitle with match name */
  if (subText) subText.textContent = ch.title || "Select stream quality";

  /* Build quality option buttons, sorted highest → lowest */
  optsCont.innerHTML = "";
  const sorted = keys.map(Number).sort((a, b) => b - a);
  sorted.forEach(height => {
    const url = q[String(height)];
    const isRec = String(height) === "720";
    const btn = document.createElement("button");
    btn.className = "quality-opt-btn" + (isRec ? " recommended" : "");
    btn.innerHTML = `
      ${isRec ? '<span class="quality-opt-badge">Recommended</span>' : ""}
      <span class="quality-opt-label">${height}p</span>
      <span class="quality-opt-desc">${QUALITY_DESC[String(height)] || height + "p"}</span>
    `;
    btn.addEventListener("click", () => {
      overlay.classList.add("hidden");
      /* Update the channel's active URL to the chosen quality */
      const selectedCh = { ...ch, url };
      activeChannel = selectedCh;
      buildQualitySelector({ ...ch, url });
      _startPlay(selectedCh, url);
    });
...
/** Internal: navigate to player view and load stream */
function _startPlay(ch, url) {
  /* Switch to player view */
  const homeInner = document.getElementById("home-view");
  const playerView = document.getElementById("player-view");
  if (homeInner) homeInner.style.display = "none";
  ...
  if (shaka.Player.isBrowserSupported()) {
    ...
    try {
      ...
      /* ── The secret sauce for Chrome HLS extensions ──
         ...
         video.src will allow the extension to intercept and play the stream,
