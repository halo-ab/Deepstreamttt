...
async function loadStream(streamUrl) {
  ...
  if (shaka.Player.isBrowserSupported()) {
    ...
    /* ── Strip Referer & Origin headers from every CDN request ──
       FanCode's Akamai CDN returns 403 when it sees a foreign Referer,
       causing Shaka Error 1002. The meta no-referrer tag only covers
       browser navigation — we must strip them here for XHR too. */
