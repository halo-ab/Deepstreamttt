...
async function handleEvents() {
  try {
    const res = await fetch(`${JSON_URL}?_=${Date.now()}`, {
      headers: { "User-Agent": "DeepStream/1.0", Accept: "application/json" },
    });
    ...
    /* Pick a direct quality URL so the client doesn't start at 1080p */
    const out = live.map((m) => {
      const autoEn = m.auto_streams?.ENGLISH?.streams || m.auto_streams?.Hindi?.streams || {};
      const streams = m.streams || {};
      const url =
        autoEn["360p"] || autoEn["480p"] || autoEn["240p"] ||
    ...
    return new Response(
      JSON.stringify({ lastUpdate: data.updatedAt || data["last update time"] || "", matches: out }),
      {
        status: 200,
        headers: {
          ...cors(),
